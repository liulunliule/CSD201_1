    ...
    void updateQueue(){
        ...
        while(p.next != null) {
            if(isPrime(p.getInfo().getColor())) {
                Laptop newLatop = new Laptop(p.getInfo().getName(), p.getInfo().getPrice()+5, p.getInfo().getColor());
                p.setInfo(newLatop);
            } else {
                Laptop newLatop = new Laptop(p.getInfo().getName(), p.getInfo().getPrice()+10, p.getInfo().getColor());
                p.setInfo(newLatop);
            }
            p=p.next;
        }
    ...
    */   
    Laptop dequeue(){
        ...
        if(isEmpty()) {
            ...
            Node destroy = front;
            Laptop retValue = destroy.getInfo();
            front = front.next;
            size--;
        return retValue; // modify the return value appropriately
